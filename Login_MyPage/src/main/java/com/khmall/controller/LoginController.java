...
public class LoginController {
	 ...
	 * 
	 * Member member = (Member) session.getAttribute("loginSession");
	 *   		└──── session loginSession 이라는 변수명에 저장된 로그인 정보를 가져오기
	 ...
	 * input에서 name = "keyword"이기 떄문에 @RequestParam("keyword") 쓴 것
	...
	@PostMapping("/search")											//String a : 소비자가 작성한 키
	public String searchMembers(Model model, @RequestParam("keyword") String keyword) {
		...
		model.addAttribute("results",member); // results:member가 담긴 그릇 member:검색으로 가져온 결과 있음
		return "search";
