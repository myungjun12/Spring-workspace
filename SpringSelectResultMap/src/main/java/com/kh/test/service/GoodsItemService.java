...
public class GoodsItemService {
	...
	/* @Autowired 는 아래오 ㅏ같은 표현을 가지고 있음
	...
	public boolean existsByName(String itemName) {
		...
		//-> resultMap 사용하지 않은 방식
		
		//-> resultMap 사용한 방식
		GoodsItem count = goodsItemMapper.existsByName(itemName); 
	  //Integer totalCount = 참인지 것지인지 ?  true이고 null이 아닐 때 : false일때 null = 0;
		...
		/*return goodsItemMapper.existsByName(itemName);
			...
			Mapper애서는 Integer을 작성하고, service에서 반납할 때는 boolean을 사용해서 타입 불일치
			Mapper랑 Service 모두 Integer = int 로 작성하거나
			Mapper랑 Service 모두 boolean 으로 작성해 */
	}
}
